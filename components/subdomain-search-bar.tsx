...
import {
  ...
  isSubdomainSearchQueryActive,
  parseQueryToBuilderRows,
  type SearchBuilderRow,
} from "@/lib/subdomain-search-query";
...
export type SubdomainSearchBarHandle = {
  openBuilder: () => void;
};

type SubdomainSearchBarProps = {
  basePath: string;
  perPage: number;
  subAll: boolean;
  initialQuery: string;
...
export const SubdomainSearchBar = forwardRef<SubdomainSearchBarHandle, SubdomainSearchBarProps>(
  function SubdomainSearchBar({ basePath, perPage, subAll, initialQuery, size = "md" }, ref) {
    ...
    const navigate = useCallback(
      (q: string) => {
        ...
        p.set("tab", "subdomains");
        p.set("perPage", String(perPage));
        if (subAll) p.set("subAll", "1");
        if (q.trim()) p.set("q", q.trim());
        router.push(`${basePath}?${p.toString()}`);
      },
      [router, basePath, perPage, subAll],
    );
    ...
    const filterApplied = isSubdomainSearchQueryActive(initialQuery);
    const showTooShortHint =
      initialQuery.trim().length > 0 && !filterApplied && !isStructuredSearchQuery(initialQuery);

    const controlTextClass = size === "sm" ? "text-[11px]" : "text-[12px]";
    const controlPadClass = size === "sm" ? "py-1.5" : "py-2";
    const inputWidthClass = size === "sm" ? "w-[min(100%,240px)]" : "w-[min(100%,280px)]";
      ...
      <div className="flex flex-col items-end gap-2">
        <form onSubmit={handleSimpleSubmit} className="flex flex-wrap items-center justify-end gap-2">
          <div className="relative">
            ...
            <input
              ...
              className={[
                "ui-input-field rounded-lg border border-line pl-8 pr-9 text-cream outline-none placeholder:text-muted focus:ring-1 focus:ring-accent/30",
                inputWidthClass,
                controlPadClass,
                controlTextClass,
              ].join(" ")}
          ...
          <button
            type="submit"
            className={[
              "rounded-lg border border-line px-3 font-medium text-cream transition-colors hover:bg-[var(--nav-hover-bg)]",
              controlPadClass,
              controlTextClass,
            ].join(" ")}
          >
          ...
          {hasActiveFilter && (
            <button
              ...
              className={[
                "rounded-lg border border-line text-muted transition-colors hover:border-warn/40 hover:bg-warn/10 hover:text-warn",
                size === "sm" ? "p-1.5" : "p-2",
              ].join(" ")}
            >
              <IconX />
        ...
        >
          ...
          <div className="space-y-3 px-5 py-4">
            {advRows.map((row, idx) => (
              <div key={row.id} className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder={idx === 0 ? "e.g. api" : "Keyword…"}
            ...
            <button
              type="button"
              onClick={() => setAdvRows([...advRows, { id: newRowId(), term: "", operator: "AND" }])}
